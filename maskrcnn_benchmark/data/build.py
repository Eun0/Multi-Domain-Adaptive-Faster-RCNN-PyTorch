...
def make_mdc_data_loader(cfg, is_train = True, is_source = True, is_distributed = False, start_iter = 0):
    ...
    if is_train:
        ...
        if cfg.MODEL.DOMAIN_ADAPTATION_ON:
            assert (
            images_per_batch % ((num_targets + 1)*num_gpus) == 0
            ), "SOLVER.IMS_PER_BATCH ({}) must be divisible by {} times the number "
            "of GPUs ({}) used.".format(images_per_batch, num_targets + 1,num_gpus)
            images_per_gpu = images_per_batch // ((num_targets+1)*num_gpus)
        shuffle = True
    ...
    DatasetCatalog = paths_catalog.MultiDomainDatasetCatalog
